...
begin
  ...
  select x.network, x.enabled, x.is_default, x.source, x.min_minutes, x.max_minutes, x.rotation_minutes, x.fake_enabled, x.fake_prefix, x.fake_count, x.status_color, coalesce(nullif(trim(x.message), ''), case x.network when 'mtn' then 'MTN orders are being processed. Please allow the estimated delivery window.' when 'telecel' then 'Telecel orders are being processed. Please allow the estimated delivery window.' when 'airteltigo' then 'AirtelTigo orders are being processed. Please allow the estimated delivery window.' else 'Your order is being processed. Please allow the estimated delivery window.' end), coalesce(x.auto_enabled, current.auto_enabled, false), coalesce(x.auto_min_minutes, current.auto_min_minutes, 30), coalesce(x.auto_max_minutes, current.auto_max_minutes, 240), coalesce(x.updated_at, now())
