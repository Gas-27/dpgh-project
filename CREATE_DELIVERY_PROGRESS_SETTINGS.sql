...
begin
  ...
  select network, enabled, is_default, source, min_minutes, max_minutes, rotation_minutes, fake_enabled, fake_prefix, fake_count, status_color, coalesce(nullif(trim(message), ''), case network when 'mtn' then 'MTN orders are being processed. Please allow the estimated delivery window.' when 'telecel' then 'Telecel orders are being processed. Please allow the estimated delivery window.' when 'airteltigo' then 'AirtelTigo orders are being processed. Please allow the estimated delivery window.' else 'Your order is being processed. Please allow the estimated delivery window.' end), auto_enabled, auto_min_minutes, auto_max_minutes, updated_at
