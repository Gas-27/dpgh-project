...
interface Service { id: string; slug: string; name: string; category: string; description: string; price: number; duration: string; icon: string; logo_url: string | null; }
const categories = [
...
export default function DigitalServicesCatalog({ onBuy }: { onBuy: (service: Service) => void }) {
  ...
  useEffect(() => { supabase.from("digital_services").select("id,slug,name,category,description,price,duration,icon,logo_url").eq("active", true).order("name").then(({ data, error }) => { if (error) toast({ title: "Services unavailable", description: "Please try again shortly.", variant: "destructive" }); setServices((data as Service[]) || []); setLoading(false); }); }, [toast]);
