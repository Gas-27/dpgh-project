...
export default function ServicePurchaseDialog({ service, onOpenChange }: { service: Service | null; onOpenChange: (open: boolean) => void }) {
  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("initialize-payment", { body: {
      ...
      metadata: { type: "service_payment", service_id: service.id, service_name: service.name, customer_phone: phone, access_pin: pin, customer_id: auth.user?.id || null, service_payment: true },
    }});
    setLoading(false);
