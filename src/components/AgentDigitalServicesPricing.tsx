...
type Price = { service_id: string; base_price: number; sell_price: number; max_price: number };
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: serviceRows, error: serviceError }, { data: priceRows }] = await Promise.all([
        supabase.from("digital_services").select("id,name,category,price,is_free,agent_min_price,agent_max_price").eq("active", true).order("category").order("name"),
