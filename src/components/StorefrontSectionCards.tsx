...
export default function StorefrontSectionCards({ active, onSelect, onBecomeAgent, hiddenIds = [] }: { active?: string; onSelect: (id: SectionId) => void; onBecomeAgent?: () => void; hiddenIds?: SectionId[] }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[700px] px-0">
      <div className="relative aspect-[347/303] w-full overflow-hidden rounded-[14px] bg-card p-1 shadow-[0_12px_30px_rgba(5,8,35,0.18)]" style={{ backgroundImage: "url(https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-tNtwT7RbsxArhGUNJXat8FqNqW9mw9.png)" }}>
        <div className="relative grid h-full grid-cols-2 grid-rows-5 gap-3 p-1 sm:gap-3.5 sm:p-1.5">
          {visibleSections.map(({ id, label, description, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`group relative min-h-0 overflow-hidden rounded-xl border border-border/40 bg-gradient-to-br p-2.5 text-left text-foreground shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:p-3 ${id === "instant" ? "from-emerald-400 via-teal-400 to-cyan-600" : id === "services" ? "from-amber-300 via-orange-400 to-yellow-600" : id === "subscription" ? "from-fuchsia-400 via-fuchsia-500 to-purple-700" : "from-cyan-400 via-sky-400 to-violet-400"} ${active === id ? "ring-2 ring-primary ring-offset-2 ring-offset-background" : ""}`}
              >
