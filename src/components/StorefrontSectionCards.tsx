...
export default function StorefrontSectionCards({ active, onSelect, onBecomeAgent, hiddenIds = [] }: { active?: string; onSelect: (id: SectionId) => void; onBecomeAgent?: () => void; hiddenIds?: SectionId[] }) {
  return <div className="mx-auto grid w-full max-w-5xl grid-cols-2 gap-3 px-0 sm:gap-4">
    {sections.filter(({ id }) => !hiddenIds.includes(id)).map(({ id, label, description, icon: Icon, tone }) => {
      ...
      return <button key={id} type="button" onClick={handleClick} aria-pressed={active === id} className={`group storefront-section-${tone} flex min-h-[74px] w-full items-center justify-between gap-2 rounded-xl border px-3 py-2.5 text-left transition hover:-translate-y-0.5 hover:brightness-110 hover:shadow-lg sm:min-h-[92px] sm:px-3.5 sm:py-3 ${active === id ? "storefront-section-active ring-2 ring-offset-1 ring-offset-background" : ""}`}>
