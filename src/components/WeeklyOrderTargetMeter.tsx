...
export default function WeeklyOrderTargetMeter({ storeId, storeType }: { storeId?: string; storeType: StoreType }) {
  ...
  async function load() {
    ...
    setTarget(Math.max(1, Number(settings?.weekly_order_target ?? 2000)));
  ...
  return <Card className="overflow-hidden border-primary/30 bg-gradient-to-br from-primary/10 via-card to-accent/10">
    <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Flame className="h-5 w-5 text-primary" /> Weekly growth target</CardTitle><p className="mt-1 text-sm text-muted-foreground">Your progress resets every Monday.</p></div><Badge variant={reached ? "default" : "secondary"}>{reached ? "Target reached" : `${orderCount.toLocaleString()} / ${target.toLocaleString()}`}</Badge></div></CardHeader>
    <CardContent className="space-y-4">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><div className="flex items-end justify-between gap-3"><div><p className="text-3xl font-bold text-primary">{orderCount.toLocaleString()}</p><p className="text-xs text-muted-foreground">orders this week</p></div><Award className="h-10 w-10 text-primary/60" /></div><Progress value={progress} className="h-3" /><div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => requestReduction("price_reduction")} disabled={!reached || Boolean(requested.price_reduction) || Boolean(requesting)}>{requesting === "price_reduction" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{requested.price_reduction ? "Request sent" : "Request price reduction"}</Button><Button size="sm" variant="outline" onClick={() => requestReduction("huge_price_reduction")} disabled={!reached || Boolean(requested.huge_price_reduction) || Boolean(requesting)}>{requested.huge_price_reduction ? "Huge request sent" : "Request huge reduction"}</Button></div>{!reached && <p className="text-xs text-muted-foreground">The request buttons unlock at {target.toLocaleString()} weekly orders.</p>}</>}</CardContent>
  </Card>;
}
