...
export default function TrackOrderDropdown({ source, storeId, onTrack, primaryColor = "#2563eb" }: Props) {
  ...
  return <div className="overflow-hidden rounded-[22px] border border-cyan-400/80 bg-gradient-to-r from-slate-950 via-blue-950 to-blue-700 shadow-lg shadow-blue-950/30">
    ...
    {open && <div className="space-y-3 border-t border-white/15 bg-slate-950/35 p-4 sm:p-5">
      <div className="flex gap-2"><Input value={lookup} onChange={(event) => setLookup(event.target.value)} onKeyDown={(event) => event.key === "Enter" && onTrack?.(lookup)} placeholder="0242206542 or order ID" className="border-cyan-400 bg-white/10 text-white placeholder:text-blue-100/60 focus-visible:ring-cyan-400" /><Button type="button" onClick={() => onTrack?.(lookup)} style={{ backgroundColor: primaryColor }}><Search className="mr-2 h-4 w-4" />Track</Button></div>
