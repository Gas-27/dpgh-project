...
export default function AgentProductStorePanel({ storeId, storeKind, supportPhone, walletBalance = 0, onWalletBalanceChange }: { storeId?: string; storeKind: StoreKind; supportPhone?: string; walletBalance?: number; onWalletBalanceChange?: (balance: number) => void }) {
  ...
  const [paymentMode, setPaymentMode] = useState<"direct" | "whatsapp">("whatsapp");
  const [paymentNumber, setPaymentNumber] = useState(supportPhone || "");
  ...
  async function addProduct() {
    ...
    const { error } = await supabase.from("store_products").insert({ store_id: storeId, store_kind: storeKind, title: title.trim(), description: description.trim(), price: Number(price), image_urls: cleanImages, payment_mode: paymentMode, whatsapp_payment_number: paymentMode === "whatsapp" ? paymentNumber.trim() : null });
  ...
  return <div className="space-y-6">
    <Card className="border-primary/30"><CardHeader><CardTitle className="flex items-center gap-2"><Store className="h-5 w-5 text-primary" /> Store products</CardTitle><p className="text-sm text-muted-foreground">Post products for sale on your storefront. Buyers can place an order and contact you through your support number{supportPhone ? ` (${supportPhone})` : ""}.</p><div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm text-muted-foreground"><p className="font-semibold text-foreground">How the product session works</p><p className="mt-1">Your first product post is free. The weekly plan gives you five active product posts for seven days, while the monthly plan gives you thirty posts for thirty days. After the included posts are used, each additional post costs GHS 1. Buyers open a dedicated product page to view the images, description, price, and WhatsApp purchase button.</p><p className="mt-2"><strong>Boost to Global Products</strong> (GHS 35) places this product near the top of the Global Products section for thirty days. <strong>Boost sitewide</strong> (GHS 40) places it near the top of product listings across the Package page and storefronts for thirty days. Boosts improve placement; they do not change the product price or availability.</p></div></CardHeader><CardContent className="space-y-4">
