...
export default function SubagentAFAPriceManager() {
  const [packages, setPackages] = useState<AFAPackage[]>([]);
  const [subagentPrices, setSubagentPrices] = useState<SubagentAFAPrice[]>([]);
  const [loading, setLoading] = useState(true);
  const [showDialog, setShowDialog] = useState(false);
  const [editingPackageId, setEditingPackageId] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [agentBasePrice, setAgentBasePrice] = useState(0);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Get subagent store ID and parent agent ID
      ...
      // Fetch subagent's current prices
  ...
  const handleOpenDialog = (packageId: string) => {
    const existingPrice = subagentPrices.find((p) => p.afa_package_id === packageId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const existingPrice = subagentPrices.find((p) => p.afa_package_id === editingPackageId);
      ...
      if (existingPrice) {
        ...
        // Create new price
        const { error } = await supabase.from("subagent_afa_prices").insert([
          {
    ...
    <div className="space-y-4">
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {packages.map((pkg) => {
              const subagentPrice = subagentPrices.find((p) => p.afa_package_id === pkg.id);
              const sellPrice = subagentPrice?.sell_price || 0;
              const commission = subagentPrice?.commission_amount || (pkg.base_price * pkg.commission_percent) / 100;
