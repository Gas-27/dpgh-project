...
export default function PackageStatusIndicator({
  ...
  const statusText = isOffline ? 'Package is Currently Offline' : 'Package Not Available';

  return (
    <div className={`flex items-center gap-2 mb-3 p-2 rounded-lg ${isOffline ? 'bg-red-50 border border-red-200' : 'bg-gray-50 border border-gray-200'}`}>
      {/* RED ALERT ICON at the top */}
      <AlertCircle className={`h-5 w-5 flex-shrink-0 ${isOffline ? 'text-red-600' : 'text-gray-600'}`} />
      
      <div className="flex-1">
        <p className={`text-sm font-semibold ${isOffline ? 'text-red-700' : 'text-gray-700'}`}>
          {statusText}
        </p>
      </div>
