...
type ProviderService = { service: number; name: string; category: string; rate: string; min: string; max: string; refill?: boolean; cancel?: boolean };
...
export default function SocialBoostPurchasePanel({ walletBalance, ownerType = "user" }: Props) {
  ...
  const services = catalog.filter((item) => item.category.toLowerCase().includes(platform.toLowerCase()) || item.name.toLowerCase().includes(platform.toLowerCase()));
