...
export default function SocialBoostPurchasePanel({ walletBalance, ownerType = "user" }: Props) {
  ...
  return <section className="space-y-4 rounded-2xl border border-blue-500/60 bg-[#071a36] p-4 text-white sm:p-5">
    <div className="flex items-center justify-between"><div><h2 className="text-xl font-bold">Social Boost Purchase</h2><p className="text-sm text-blue-100/70">Wallet balance: GHC {Number(walletBalance).toFixed(2)}</p></div><span className="rounded-full bg-cyan-400/15 px-3 py-1 text-sm font-bold text-cyan-200">GHC {total.toFixed(2)}</span></div>
