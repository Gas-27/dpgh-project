...
type Setting = { network: string; enabled: boolean; is_default?: boolean; source: "manual" | "orders" | "fake"; min_minutes: number | string; max_minutes: number | string; rotation_minutes: number | string; fake_enabled: boolean; fake_prefix: string; fake_count: number | string; skip_contacts?: string[]; status_color: "green" | "red" | "yellow"; message: string };
...
export default function DeliveryProgressAdmin() {
  ...
  const load = async () => {
    ...
    const [{ data: settingRows, error: settingsError }, { data: orderRows, error: ordersError }] = await Promise.all([
      supabase.from("delivery_progress_settings").select("network, enabled, is_default, source, min_minutes, max_minutes, rotation_minutes, fake_enabled, fake_prefix, fake_count, skip_contacts, status_color, message").order("network"),
  ...
  const skipContacts = (current?.skip_contacts ?? []).map((value) => value.replace(/\D/g, "")).filter(Boolean);
  const skippedSelectedOrders = selectedOrders.filter((id) => { const order = orders.find((item) => item.id === id); return skipContacts.some((contact) => String(order?.customer_number || "").replace(/\D/g, "").endsWith(contact)); });
  const updateableSelectedOrders = selectedOrders.filter((id) => !skippedSelectedOrders.includes(id));
  ...
  const save = async () => {
    if (!current) return;
      setSaving(true);
    const normalized = settings.map((setting) => ({ ...setting, min_minutes: Math.max(2, Number(setting.min_minutes) || 2), max_minutes: Math.max(Math.max(2, Number(setting.min_minutes) || 2), Number(setting.max_minutes) || 2), rotation_minutes: Math.max(1, Number(setting.rotation_minutes) || 1), fake_count: Math.min(100, Math.max(1, Number(setting.fake_count) || 1)), skip_contacts: (setting.skip_contacts ?? []).flatMap((contact) => String(contact).split(/[\s,;]+/)).map((contact) => contact.replace(/\D/g, "")).filter(Boolean), fake_prefix: setting.fake_prefix.split(",").map((prefix) => prefix.replace(/\D/g, "").slice(0, 3)).filter(Boolean).join(", "), source: setting.fake_enabled ? "fake" : setting.source === "fake" ? "orders" : setting.source, is_default: setting.network === defaultNetwork, updated_at: new Date().toISOString() }));
    ...
    if (updateableSelectedOrders.length > 0) {
      const result = await supabase.from("orders").update({ order_status: targetStatus, fulfillment_status: targetStatus, updated_at: new Date().toISOString() }).in("id", updateableSelectedOrders);
      statusError = result.error;
    }
    setSaving(false);
    if (statusError) { toast({ title: "Progress saved, statuses failed", description: statusError.message, variant: "destructive" }); return; }
    toast({ title: "Delivery progress settings saved", description: selectedCount > 0 ? `${updateableSelectedOrders.length} order${updateableSelectedOrders.length === 1 ? "" : "s"} changed to ${targetStatus}; ${skippedSelectedOrders.length} excluded order${skippedSelectedOrders.length === 1 ? " was" : "s were"} skipped.` : "Your network delivery progress changes were saved." });
    setSelectedOrders([]);
    await load();
  };
