...
export default function DeliveryProgressAdmin() {
  ...
  const save = async () => {
    if (!current) return;
      setSaving(true);
    const normalized = settings.map((setting) => ({ ...setting, min_minutes: Math.max(2, Number(setting.min_minutes) || 2), max_minutes: Math.max(Math.max(2, Number(setting.min_minutes) || 2), Number(setting.max_minutes) || 2), rotation_minutes: Math.max(1, Number(setting.rotation_minutes) || 1), fake_count: Math.min(100, Math.max(1, Number(setting.fake_count) || 1)), skip_contacts: (setting.skip_contacts ?? []).map((contact) => String(contact).replace(/\D/g, "")).filter(Boolean), fake_prefix: setting.fake_prefix.split(",").map((prefix) => prefix.replace(/\D/g, "").slice(0, 3)).filter(Boolean).join(", "), source: setting.fake_enabled ? "fake" : setting.source === "fake" ? "orders" : setting.source, is_default: setting.network === defaultNetwork, updated_at: new Date().toISOString() }));
