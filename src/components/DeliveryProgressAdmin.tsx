...
export default function DeliveryProgressAdmin() {
  ...
  const save = async () => {
    ...
    const { error: statusError } = await supabase.from("orders").update({ order_status: targetStatus, fulfillment_status: targetStatus, updated_at: new Date().toISOString() }).in("id", updateableSelectedOrders);
    setSaving(false);
    if (statusError) { toast({ title: "Progress saved, statuses failed", description: statusError.message, variant: "destructive" }); return; }
    toast({ title: `${labels[selectedNetwork]} progress updated`, description: `${updateableSelectedOrders.length} order${updateableSelectedOrders.length === 1 ? "" : "s"} changed to ${targetStatus}; ${skippedSelectedOrders.length} excluded order${skippedSelectedOrders.length === 1 ? " was" : "s were"} skipped. Unselected orders were left unchanged.` });
    setSelectedOrders([]);
    await load();
  };
