...
type SmsAudience = "all" | "recent" | "inactive" | "custom";
type Sender = {
  id: string;
  sender_id: string;
...
export default function SmsComposer({ ownerType, ownerId, storeUrl: providedStoreUrl, publicMode = false, hideSenderPhone = false }: SmsComposerProps) {
  ...
  const loadAudience = async (nextAudience: SmsAudience) => {
    ...
    try {
      ...
      if (error) throw error;
      const inactiveDays = Math.max(1, Number(customInactiveDays) || 30);
      const cutoff = Date.now() - (nextAudience === "custom" ? inactiveDays : 30) * 24 * 60 * 60 * 1000;
      const phones = new Set<string>();
      for (const order of data || []) {
        const phone = normalizeGh(String(order.customer_number || ""));
        if (!phone) continue;
        const recent = new Date(order.created_at).getTime() >= cutoff;
        if ((nextAudience === "recent" && recent) || ((nextAudience === "inactive" || nextAudience === "custom") && !recent) || nextAudience === "all") phones.add(phone);
      }
    ...
    <div className="flex flex-wrap gap-2" role="tablist" aria-label="Customer audience">
      {(["all", "recent", "inactive", "custom"] as SmsAudience[]).map((item) => (
        <Button key={item} type="button" size="sm" variant={audience === item ? "default" : "outline"} disabled={audienceLoading} onClick={() => void loadAudience(item)}>
          {item === "all" ? "All customers" : item === "recent" ? "Bought in 30 days" : item === "inactive" ? "No purchase in 30 days" : "Custom"}
        </Button>
      ))}
    </div>
