...
export default function DomainDashboardPanel() {
  ...
  async function search() {
    ...
    if (!/^([a-z0-9-]+\.)+[a-z]{2,}$/.test(value)) { setError("Enter a complete domain, for example example.com."); return; }
    setLoading(true); setError(""); setMessage(""); setResult(null);
    try { const data = await call("POST", "/v1/domains/available", { domains: [value] }); setResult(data?.domains?.[0] ?? data?.items?.[0] ?? data); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Domain search failed."); } finally { setLoading(false); }
  }

  async function loadDomains() {
    setLoading(true); setError("");
    try { const data = await call("GET", "/v1/domains", undefined, { take: "100", skip: "0", orderBy: "-expirationDate" }); setDomains(data?.items ?? []); setMessage("Domains loaded from Spaceship."); }
  ...
  async function buy() {
    if (!result?.domain && !query.trim()) return;
    const value = result?.domain || query.trim().toLowerCase();
    setLoading(true); setError(""); setMessage("");
    try { const data = await call("POST", `/v1/domains/${encodeURIComponent(value)}`); setMessage(data?.operationId ? `Purchase started. Operation: ${data.operationId}` : "Purchase request submitted to Spaceship."); await loadDomains(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Purchase request failed."); } finally { setLoading(false); }
  }

  async function saveDns() {
    setLoading(true); setError("");
    try { await call("PUT", `/v1/dns/records/${encodeURIComponent(domain)}`, { records }); setMessage("DNS records saved to Spaceship."); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Could not save DNS records."); } finally { setLoading(false); }
  }

  return <div className="space-y-6">
    <Card className="border-primary/30 bg-primary/5"><CardHeader><CardTitle className="flex items-center gap-2"><Globe2 className="h-5 w-5 text-primary" /> Domains</CardTitle><p className="text-sm text-muted-foreground">Search, buy, and manage Spaceship domains, DNS records, and nameservers from one place.</p></CardHeader><CardContent className="space-y-4">
      ...
      {result && <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-primary/30 p-4"><div><p className="font-semibold">{result.domain || query}</p><Badge variant="secondary">{result.available === true || result.result === "available" ? "Available" : "Unavailable"}</Badge></div><Button onClick={buy} disabled={loading || !(result.available === true || result.result === "available")}>Buy domain</Button></div>}
    </CardContent></Card>

    <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Your Spaceship domains</CardTitle><p className="text-sm text-muted-foreground">Load registered domains and open DNS management.</p></div><Button variant="outline" onClick={loadDomains} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div></CardHeader><CardContent>{domains.length ? <div className="space-y-2">{domains.map((item, index) => { const name = item.name || item.domain; return <div key={`${name}-${index}`} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border p-3"><span className="font-medium">{name}</span><Button variant="outline" size="sm" onClick={() => manage(name)}>Manage DNS</Button></div>; })}</div> : <p className="text-sm text-muted-foreground">Click Refresh to load your domains.</p>}</CardContent></Card>
    ...
    {message && <p className="flex items-center gap-2 text-sm text-primary"><CheckCircle2 className="h-4 w-4" />{message}</p>}{error && <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive"><strong>Spaceship request failed:</strong> {error}</div>}
  </div>;
}
