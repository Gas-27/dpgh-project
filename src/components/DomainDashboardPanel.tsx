...
export default function DomainDashboardPanel({ walletBalance = 0, walletLabel = "Wallet balance", agentStoreId = null, storeKind = "agent", onPurchaseComplete }: DomainDashboardPanelProps) {
  ...
  async function loadDomains() {
    ...
    try { const userId = (await supabase.auth.getUser()).data.user?.id; let query = supabase.from("domain_purchases").select("id, domain, status, price, provider_operation_id, assigned_domain, created_at"); query = agentStoreId ? query.eq("agent_store_id", agentStoreId) : query.eq("buyer_user_id", userId ?? "00000000-0000-0000-0000-000000000000"); const { data, error: loadError } = await query.order("created_at", { ascending: false }); if (loadError) throw loadError; setDomains(data ?? []); setMessage("Owned domains loaded successfully."); }
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Your domains</CardTitle><p className="text-sm text-muted-foreground">Your purchase remains visible here while admin completes registration and store setup.</p></div><Button variant="outline" onClick={loadDomains} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div></CardHeader><CardContent className="space-y-3">{domains.length ? <div className="space-y-2">{domains.map((item, index) => { const name = item.name || item.domain; const status = item.status || "pending"; return <div key={`${name}-${index}`} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border p-3"><div><p className="font-medium">{name}</p><p className="text-xs text-muted-foreground">Status: {status === "pending" || status === "pending_manual" ? "Pending admin setup" : status}</p></div><Badge variant="outline">Admin managed</Badge></div>; })}</div> : <p className="text-sm text-muted-foreground">Click Refresh to load your domains.</p>}<div className="rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground"><p className="font-semibold text-foreground">Admin will take care of the setup</p><p>Admin will purchase the domain, connect it to your store, add it to Vercel and Google Cloud, and complete the required DNS setup. This normally takes up to 24 hours or less. You do not need to manage DNS yourself.</p></div></CardContent></Card>
