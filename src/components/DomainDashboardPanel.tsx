...
export default function DomainDashboardPanel({ walletBalance = 0, walletLabel = "Wallet", agentStoreId = null, storeKind = "agent", onPurchaseComplete }: DomainDashboardPanelProps) {
  ...
  async function loadDomains() {
    ...
    try { const userId = (await supabase.auth.getUser()).data.user?.id; let query = supabase.from("domain_purchases").select("id, domain, status, price, provider_operation_id, assigned_domain, created_at, purchased_at, term_ends_at, renewal_due_at, renewal_status, auto_renew, renewal_price, custom_domain_enabled"); query = agentStoreId ? query.eq("agent_store_id", agentStoreId) : query.eq("buyer_user_id", userId ?? "00000000-0000-0000-0000-000000000000"); const { data, error: loadError } = await query.order("created_at", { ascending: false }); if (loadError) throw loadError; setDomains(data ?? []); setMessage("Owned domains loaded successfully."); }
  ...
  useEffect(() => { void loadDomains(); }, [agentStoreId]);

  async function renewDomain(item: any) {
    setRenewingId(item.id); setError(""); setMessage("");
    try {
      const { error: renewError } = await supabase.rpc("retry_domain_renewal", { p_domain_purchase_id: item.id });
      if (renewError) throw renewError;
      setMessage(`${item.assigned_domain || item.domain} renewed for another year.`);
      await loadDomains();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not renew domain."); } finally { setRenewingId(null); }
  }
  ...
  async function buy(selected?: string) {
    ...
    try {
      ...
      const { data: pricingRows, error: pricingError } = await supabase.from("spaceship_tld_pricing").select("tld,customer_price,active");
      if (pricingError) throw pricingError;
      const pricing = (pricingRows ?? []).find((row: any) => String(row.tld ?? "").trim().toLowerCase().replace(/^\.?/, ".") === tld);
  ...
  return <div className="space-y-6">
    <Card className="border-primary/30 bg-primary/5"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Globe2 className="h-5 w-5 text-primary" /> Domains</CardTitle><p className="text-sm font-medium text-slate-700">Purchase your domain now to have your own branded store address.</p></div><Badge variant="outline">{walletLabel}: GHC {walletBalance.toFixed(2)}</Badge></div></CardHeader><CardContent className="space-y-4">
      ...
      {results.length > 0 && <div className="grid gap-2 sm:grid-cols-2">{results.map((item: any) => { const name = item.domain || item.name || item.domainName || query; const available = isAvailable(item); const nameParts = String(name).trim().toLowerCase().split(".").filter(Boolean); const tld = nameParts.length > 1 ? `.${nameParts.slice(1).join(".")}` : ""; const configured = tldPricing[tld]; const price = configured?.customer_price ?? 0; const hasConfiguredPrice = Number.isFinite(price) && price > 0 && configured?.active !== false; const purchasable = available && hasConfiguredPrice; const canAfford = purchasable && walletBalance >= price; return <div key={name} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-primary/30 p-4"><div><p className="font-semibold">{name}</p><div className="flex flex-wrap items-center gap-2"><Badge variant={available ? "default" : "secondary"}>{available ? "Available" : "Domain taken"}</Badge><span className="text-sm font-semibold text-primary">{hasConfiguredPrice ? `GHC ${price.toFixed(2)}` : "Price not set"}</span></div>{!available && hasConfiguredPrice && <p className="text-xs text-muted-foreground">This domain is taken, but the admin price is shown for reference.</p>}{available && !hasConfiguredPrice && <p className="text-xs text-muted-foreground">This extension is not currently configured for purchase.</p>}</div><Button onClick={() => { setResult(item); setQuery(name); void buy(name); }} disabled={loading || !canAfford}>{!available ? "Unavailable" : !hasConfiguredPrice ? "Unavailable" : canAfford ? "Buy domain" : "Insufficient wallet"}</Button></div>; })}</div>}
    </CardContent></Card>

    <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Your domains</CardTitle><p className="text-sm text-muted-foreground">Your purchase remains visible here while admin completes registration and store setup.</p></div><Button variant="outline" onClick={loadDomains} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div></CardHeader><CardContent className="space-y-3">{domains.length ? <div className="space-y-2">{domains.map((item, index) => { const name = item.assigned_domain || item.domain; const status = item.term_ends_at && new Date(item.term_ends_at) < new Date() ? "expired" : item.status || "pending"; const due = item.renewal_due_at ? new Date(item.renewal_due_at) : null; const end = item.term_ends_at ? new Date(item.term_ends_at) : null; const dueSoon = Boolean(due && due <= new Date()); const canRenew = Boolean(item.assigned_domain || item.domain) && (item.renewal_status === "failed" || dueSoon || status === "expired"); return <div key={`${name}-${index}`} className="space-y-3 rounded-lg border border-border p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{name}</p><p className="text-xs text-muted-foreground">Status: {status === "pending" || status === "pending_manual" ? "Pending admin setup" : status}{end ? ` · Term ends ${end.toLocaleDateString()}` : ""}</p>{dueSoon && status !== "expired" && <p className="text-xs font-medium text-amber-600">Renewal reminder: month 11 has arrived.</p>}{item.renewal_status === "failed" && <p className="text-xs font-medium text-destructive">Auto-renew failed because your wallet did not have enough funds. Top up your wallet, then tap Auto-renew to restore this domain.</p>}{status === "expired" && <p className="text-xs font-medium text-destructive">Custom domain disabled until renewal. Your default store link still works.</p>}</div><div className="flex items-center gap-2"><Badge variant="outline">Auto-renew on</Badge>{canRenew && (dueSoon || status === "expired") && <Button size="sm" onClick={() => void renewDomain(item)} disabled={renewingId === item.id}>{renewingId === item.id ? "Renewing..." : `Renew · GHC ${Number(item.renewal_price ?? item.price ?? 0).toFixed(2)}`}</Button>}</div></div><p className="text-xs text-muted-foreground">Admin manages registration, Vercel, Google Cloud, and store connection. Setup and DNS changes may take up to 24 hours or less.</p></div>; })}</div> : <p className="text-sm text-muted-foreground">Click Refresh to load your domains.</p>}<div className="rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground"><p className="font-semibold text-foreground">Admin will take care of the setup</p><p>Admin will purchase the domain, connect it to your store, add it to Vercel and Google Cloud, and complete the required DNS setup. This normally takes up to 24 hours or less. You do not need to manage DNS yourself.</p></div></CardContent></Card>
    ...
    {message && <p className="flex items-center gap-2 text-sm text-primary"><CheckCircle2 className="h-4 w-4" />{message}</p>}{error && <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive"><strong>Domain action failed:</strong> {error}</div>}
  </div>;
}
