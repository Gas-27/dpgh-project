...
export default function DomainDashboardPanel({ walletBalance = 0, walletLabel = "Wallet balance", agentStoreId = null, storeKind = "agent", onPurchaseComplete }: DomainDashboardPanelProps) {
  ...
  return <div className="space-y-6">
    ...
    {message && <p className="flex items-center gap-2 text-sm text-primary"><CheckCircle2 className="h-4 w-4" />{message}</p>}{error && <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive"><strong>Wallet purchase failed:</strong> {error}</div>}
  </div>;
}
