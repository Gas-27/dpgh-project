...
export default function DomainDashboardPanel({ walletBalance = 0, walletLabel = "Wallet balance", agentStoreId = null, storeKind = "agent", onPurchaseComplete }: DomainDashboardPanelProps) {
  ...
  return <div className="space-y-6">
    <Card className="border-primary/30 bg-primary/5"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Globe2 className="h-5 w-5 text-primary" /> Domains</CardTitle><p className="text-sm text-muted-foreground">Search domains here, reserve one with your wallet, then admin completes the manual Spaceship purchase.</p></div><Badge variant="outline">{walletLabel}: GHC {walletBalance.toFixed(2)}</Badge></div></CardHeader><CardContent className="space-y-4">
    ...
    <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Your domains</CardTitle><p className="text-sm text-muted-foreground">Your purchase remains visible here while admin completes registration and store setup.</p></div><Button variant="outline" onClick={loadDomains} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div></CardHeader><CardContent className="space-y-3">{domains.length ? <div className="space-y-2">{domains.map((item, index) => { const name = item.assigned_domain || item.domain; const status = item.term_ends_at && new Date(item.term_ends_at) < new Date() ? "expired" : item.status || "pending"; const due = item.renewal_due_at ? new Date(item.renewal_due_at) : null; const end = item.term_ends_at ? new Date(item.term_ends_at) : null; const dueSoon = due && due <= new Date(); const canRenew = Boolean(item.assigned_domain || item.domain) && !["pending", "pending_manual"].includes(item.status); return <div key={`${name}-${index}`} className="space-y-3 rounded-lg border border-border p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{name}</p><p className="text-xs text-muted-foreground">Status: {status === "pending" || status === "pending_manual" ? "Pending admin setup" : status}{end ? ` · Term ends ${end.toLocaleDateString()}` : ""}</p>{dueSoon && status !== "expired" && <p className="text-xs font-medium text-amber-600">Renewal reminder: month 11 has arrived.</p>}{status === "expired" && <p className="text-xs font-medium text-destructive">Custom domain disabled until renewal. Your default store link still works.</p>}</div><div className="flex items-center gap-2"><Badge variant="outline">{item.auto_renew ? "Auto-renew on" : "Auto-renew off"}</Badge>{canRenew && (dueSoon || status === "expired") && <Button size="sm" onClick={() => void renewDomain(item)} disabled={renewingId === item.id}>{renewingId === item.id ? "Renewing..." : `Renew · GHC ${Number(item.renewal_price ?? item.price ?? 0).toFixed(2)}`}</Button>}</div></div><p className="text-xs text-muted-foreground">Admin manages registration, Vercel, Google Cloud, and store connection. Setup and DNS changes may take up to 24 hours or less.</p></div>; })}</div> : <p className="text-sm text-muted-foreground">Click Refresh to load your domains.</p>}<div className="rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground"><p className="font-semibold text-foreground">Admin will take care of the setup</p><p>Admin will purchase the domain, connect it to your store, add it to Vercel and Google Cloud, and complete the required DNS setup. This normally takes up to 24 hours or less. You do not need to manage DNS yourself.</p></div></CardContent></Card>
