...
export default function SubagentYouTubeSection() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('afa_settings_youtube_subagent')
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          if (payload.new && 'subagent_youtube_url' in payload.new) {
            setYoutubeUrl(payload.new.subagent_youtube_url || null);
          }
        }
      )
      .subscribe();
  ...
  const fetchYouTubeUrl = async () => {
    try {
      const { data } = await supabase
        .from('afa_settings')
        .select('subagent_youtube_url')
        .single();

      if (data) {
        setYoutubeUrl(data.subagent_youtube_url || null);
      }
    } catch (error) {
