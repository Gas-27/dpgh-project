...
type RequestRow = { id: string; requester_type: string; requester_store_id: string; requester_store_name?: string | null; store_name?: string | null; topup_reference?: string | null; top_up_reference?: string | null; request_type: string; order_count: number; target_orders: number; personalized_target_orders?: number | null; status: string; created_at: string };
...
export default function PriceReductionRequestsAdmin() {
  ...
  async function review(request: RequestRow, status: "approved" | "rejected") {
    setBusy(request.id);
    const personalized = status === "approved" ? Math.ceil(Number(request.target_orders) * 3) : null;
