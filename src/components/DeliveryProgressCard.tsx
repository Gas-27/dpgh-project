...
export default function DeliveryProgressCard({ selectedNetwork }: { selectedNetwork?: string }) {
  ...
  return <Card className="delivery-progress-card relative left-1/2 mb-4 w-[calc(100%+2rem)] -translate-x-1/2 overflow-hidden border-violet-400/40 bg-violet-950 text-white shadow-lg sm:static sm:mx-auto sm:w-full sm:max-w-6xl sm:translate-x-0"><CardHeader className="px-4 pb-2 pt-3 sm:px-5"><div className="flex items-center justify-between gap-2"><CardTitle className="flex items-center gap-2 text-base text-white sm:text-lg"><Truck className="size-4 text-primary" />Delivery Progress</CardTitle><Select value={network} onValueChange={(v) => {
              networkWasSelectedRef.current = true;
              setNetwork(v as Network);
            }}><SelectTrigger aria-label="Select delivery network" className="h-9 w-40 border-2 border-cyan-300 bg-white font-semibold text-slate-950 shadow-sm sm:w-48"><SelectValue placeholder="Select network" /></SelectTrigger><SelectContent>{Object.entries(labels).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent></Select></div></CardHeader><CardContent className="flex flex-col gap-2 px-4 pb-4 sm:px-5"><div className="delivery-status-message flex gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-2.5"><span className={`mt-1 size-2.5 shrink-0 rounded-full ${item.status_color === "red" ? "bg-red-500" : item.status_color === "yellow" ? "bg-yellow-400" : "bg-emerald-500"}`} /><div><p className="text-xs font-medium leading-4 sm:text-sm">{item.message?.trim() || `${labels[network]} orders are being processed. Please allow the estimated delivery window.`}</p><p className="mt-1 text-xs text-violet-200">Estimated delivery: <strong className="text-white">{windowText(estimateMin)} – {windowText(estimateMax)}</strong></p></div></div><div className="flex gap-2 rounded-lg border border-border bg-background/70 p-2.5"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-emerald-600" /><div className="min-w-0 text-xs sm:text-sm">{shown ? <><p><strong className="text-emerald-700">Last delivered:</strong> <span className="font-medium">{masked(shown.customer_number)}</span></p><p className="text-muted-foreground">Placed {date(shown.created_at)} · Delivered {date(shown.updated_at)} · Took <strong className="text-foreground">{duration(shown.created_at, shown.updated_at)}</strong></p></> : <p className="text-muted-foreground">No delivered {labels[network]} order found yet. This card reads delivery status directly from the orders table.</p>}</div><Badge variant="outline" className="ml-auto flex h-fit shrink-0 gap-1 border-slate-300 bg-white px-2 text-slate-950"><Clock3 className="size-3" />{labels[network]}</Badge></div><Button variant="ghost" size="sm" className="w-fit gap-2 px-0 text-xs text-muted-foreground" onClick={() => setOpen(!open)}><Info className="size-4" />What should you know?<ChevronDown className={`size-4 ${open ? "rotate-180" : ""}`} /></Button>{open && <p className="text-xs leading-5 text-muted-foreground">This is an estimated delivery window, not a guarantee. Track your own order from the Track Order bar for its actual status.</p>}</CardContent></Card>;
}
