...
export default function AdminOrderStatusUpdater() {
  ...
  const canSearch = Boolean(from && to && fromStatus && toStatus && from <= to);
  ...
  const preview = async () => {
    ...
    setLoading(true);
    let query = supabase.from("orders").select("id, network, order_status, created_at").gte("created_at", `${from}T00:00:00.000Z`).lte("created_at", `${to}T23:59:59.999Z`).order("created_at", { ascending: false }).limit(5000);
  ...
  return <Card className="border-border">
    <CardHeader><CardTitle className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-primary" />Bulk order delivery status</CardTitle><p className="text-sm text-muted-foreground">Preview and update orders directly in the orders table by network, creation time, and current status.</p></CardHeader>
