...
export default function SubagentPricesManager({ agentStoreId, packages, agentPrices, onPricesSaved }: SubagentPricesManagerProps) {
  ...
  const filteredPackages = packages.filter(p => networkFilter === "mtn_mashup" ? (p.network === "mtn_mashup" || p.network === "mashup") && p.active !== false : p.network === networkFilter && p.active !== false);
