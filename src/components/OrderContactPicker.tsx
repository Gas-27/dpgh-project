...
export default function OrderContactPicker({ ownerType, ownerId, onContacts }: Props) {
  ...
  const [orderStatus, setOrderStatus] = useState<"all" | "delivered" | "processing" | "failed" | "refunded">("all");
  ...
  const fetchContacts = async () => {
    ...
    let query = supabase.from("orders").select("customer_number,created_at,status,fulfillment_status,order_status").limit(5000);
    if (audience === "all") query = query.gte("created_at", formatDate(window.from)).lte("created_at", formatDate(window.to));
    let data: { customer_number: string | null; created_at: string | null; status: string | null; fulfillment_status: string | null; order_status: string | null }[] = [];
    ...
    const matchesStatus = (row: { status: string | null; fulfillment_status: string | null; order_status: string | null }) => {
      if (orderStatus === "all") return true;
      const fulfillment = (row.fulfillment_status || "").toLowerCase();
      const status = (row.status || "").toLowerCase();
      const orderStatusValue = (row.order_status || "").toLowerCase();
      if (orderStatus === "delivered") return fulfillment === "completed" || status === "completed" || orderStatusValue === "completed";
      if (orderStatus === "processing") return fulfillment === "processing" || status === "processing" || orderStatusValue === "processing";
      if (orderStatus === "failed") return fulfillment === "failed" || status === "failed" || orderStatusValue === "failed";
      return fulfillment === "refunded" || status === "refunded" || orderStatusValue === "refunded";
    };
    const filtered = data
      .filter((row) => audience === "all" || (audience === "recent" ? new Date(row.created_at || 0).getTime() >= cutoff : new Date(row.created_at || 0).getTime() < cutoff))
  ...
  return <div className="space-y-3 rounded-lg border border-primary/20 bg-primary/5 p-3">
    ...
    <div className="flex flex-col gap-2 sm:flex-row">
      ...
      <Select value={orderStatus} onValueChange={(value) => setOrderStatus(value as typeof orderStatus)}><SelectTrigger className="sm:max-w-48" aria-label="Order status"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All order statuses</SelectItem><SelectItem value="delivered">Delivered</SelectItem><SelectItem value="processing">Processing</SelectItem><SelectItem value="failed">Failed</SelectItem><SelectItem value="refunded">Refunded</SelectItem></SelectContent></Select>
