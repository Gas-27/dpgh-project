...
export default function OrderContactPicker({ ownerType, ownerId, onContacts }: Props) {
  ...
  const fetchContacts = async () => {
    ...
    setLoading(true);
    const { data, error } = await supabase.from("orders").select("customer_number").eq(ownerColumn, ownerId).gte("created_at", formatDate(window.from)).lte("created_at", formatDate(window.to)).limit(5000);
    setLoading(false);
