...
const SubSubagentDashboard = () => {
  ...
  const handleRequestWithdrawal = async () => {
    ...
    try {
      ...
      try {
        response = await fetch("https://uloaiqmknsrknqikbmtb.supabase.co/functions/v1/create-payout-request", {
          method: "POST",
      ...
      if (!response.ok || data.success === false) throw new Error(data.error || data.message || `Withdrawal failed (${response.status})`);
