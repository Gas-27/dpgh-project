...
const AgentDashboard = () => {
  ...
  // Available balance after accounting for pending withdrawals
  ...
  const calcStats = (os: Order[], ps: DataPackage[], bal: number): ProfitStats => {
    ...
    os.forEach(o => {
      if (o.status === "completed" || o.status === "paid") {
        rev += Number(o.amount);
        const p = ps.find(x => x.id === o.package_id);
        if (p) cost += p.agent_price;
      }
    });
  ...
  useEffect(() => {
    ...
    const c3 = supabase.channel("agent-price-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "agent_package_prices", filter: `agent_store_id=eq.${store.id}` }, () => { fetchAllData(); toast({ title: "Prices updated" }); })
      .subscribe();
  ...
  useEffect(() => { if (orders.length > 0 && packages.length > 0) setProfitStats(calcStats(orders, packages, store?.wallet_balance ?? 0)); }, [orders, packages, store?.wallet_balance]);
  ...
  const toggleNotificationActive = async (id: string, cur: boolean) => {
    const { error } = await supabase.from("agent_notifications" as any).update({ is_active: !cur }).eq("id", id);
  ...
  const handleBuyConfirm = async () => {
    ...
    // ── Pending-withdrawal balance guard ──────────────────────────────────
    if (buyPaymentMethod === "wallet") {
      ...
      if (balanceAfterBuy < pendingWithdrawalAmount) {
        toast({
          ...
          description: `You have a pending withdrawal of GH₵ ${pendingWithdrawalAmount.toFixed(2)}. Buying this package would leave your balance too low once the withdrawal is processed. Please top up your wallet or wait for the withdrawal to complete.`,
          variant: "destructive",
        });
  ...
  const handleWithdraw = async () => {
    if (!store) return;
    if (hasPendingWithdrawal) { toast({ title: "Pending withdrawal exists", description: "Wait for it to complete before requesting another.", variant: "destructive" }); return; }
  ...
  // Capture the flyer at full resolution by temporarily removing CSS scale
  ...
  // ── Bulletproof share: tries file share → text share → clipboard → download ──
  const shareFlyer = async () => {
    ...
    // Helper: download image silently
    const triggerDownload = () => {
      try {
        const a = document.createElement("a"); a.download = "flyer.png"; a.href = dataUrl!; a.click();
      } catch { /* ignore */ }
    };

    // 1. Try full Web Share API with file (Android Chrome, iOS Safari ≥15.1)
    try {
      ...
      if (e.name === "AbortError") { setGeneratingFlyer(false); return; } // user cancelled — stop
      // continue to next fallback
    }

    // 2. Text-only Web Share API (no file) + auto-download image
    try {
      if (typeof navigator.share === "function") {
        await navigator.share({ title: `${store?.store_name} – Data Bundles`, text: shareText });
        triggerDownload();
        toast({ title: "Text shared!", description: "The flyer image has been saved to your device — attach it in WhatsApp." });
        setGeneratingFlyer(false); return;
      }
    } catch (e: any) {
      if (e.name === "AbortError") { setGeneratingFlyer(false); return; }
    }

    // 3. Desktop fallback: copy text + download image
    try { await navigator.clipboard.writeText(shareText); } catch { /* ignore */ }
    triggerDownload();
    toast({ title: "Share text copied & image downloaded!", description: "Paste the text and attach the flyer image in WhatsApp or wherever you like." });
  ...
  // ─── FLYER PACKAGE CARD ───────────────────────────────────────────────────
  // All sizes in native canvas pixels (1080 wide)
      ...
      <div className="container py-8 space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="overview" className="space-y-6 mt-0">
            ...
            {/* Orders table with Load More */}
            <Card className="border-border">
              <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <CardTitle className="font-display text-lg">
                  Orders <span className="text-sm font-normal text-muted-foreground">({orders.length} of {ordersTotal} shown)</span>
                </CardTitle>
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  <>
                    <div className="overflow-x-auto">
                      <Table>
                        <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Number</TableHead><TableHead>Network</TableHead><TableHead>Size</TableHead><TableHead>Sell Price</TableHead><TableHead>Base Cost</TableHead><TableHead>Profit</TableHead><TableHead>Method</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                        <TableBody>
          ...
          <TabsContent value="buy" className="space-y-4 mt-0">
              ...
              <Card className={`border-border ${hasPendingWithdrawal ? "border-orange-500/30 bg-orange-500/5" : "bg-secondary/30"}`}>
                <CardContent className="p-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-primary" /><span className="font-medium">Wallet Balance:</span></div>
                    <span className="font-display text-xl font-bold text-primary">GH₵ {store.wallet_balance?.toFixed(2) ?? "0.00"}</span>
                  </div>
                  {hasPendingWithdrawal && (
                    <p className="text-xs text-orange-400">⚠️ GH₵ {pendingWithdrawalAmount.toFixed(2)} is reserved for a pending withdrawal. Effective spendable balance: <strong>GH₵ {effectiveBalance.toFixed(2)}</strong></p>
                  )}
                </CardContent>
              </Card>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {filteredPackages.map(pkg => {
                ...
                return (
                  <Card key={pkg.id} className={`border-border transition-all ${wouldUnderflow ? "opacity-50" : "hover:border-primary/50"}`}>
                    <CardContent className="p-4 text-center space-y-3">
                      ...
                      <p className="text-lg font-bold text-primary">GH₵ {ap.toFixed(2)}</p>
                      ...
                      <Button variant="hero" size="sm" className="w-full" onClick={() => openBuyDialog(pkg)} disabled={wouldUnderflow}>Buy Now</Button>
                    </CardContent>
                  </Card>
                );
              })}
          ...
          <TabsContent value="store" className="space-y-4 mt-0">
            ...
            <p className="text-sm text-muted-foreground">Your profit = Selling Price - Base Price.</p>
            ...
            <div className="space-y-4">
              <Card className="border-border">
                <CardHeader className="pb-3">
                  ...
                  <p className="text-sm text-muted-foreground">Live prices auto-populate. Customise colours, edit the share message, then download or share directly to WhatsApp.</p>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* Colour pickers */}
                  <div className="flex flex-wrap gap-4 items-center justify-between">
                    <div className="flex flex-wrap gap-3 items-center">
                      {([{ label: "MTN", key: "mtnColor" }, { label: "Airtel", key: "airtelColor" }, { label: "Telecel", key: "telecelColor" }, { label: "Brand", key: "buttonBg" }] as { label: string; key: keyof typeof flyerColors }[]).map(({ label, key }) => (
                  ...
                  <div className="space-y-1">
                    <Label className="text-sm font-medium">Share Message <span className="text-muted-foreground font-normal text-xs">(editable)</span></Label>
              ...
              {/* ── Flyer preview: scales to fit full container width ── */}
              <div
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 32px", backgroundColor: "#0a0a0a", borderBottom: "1px solid #1e1e1e" }}>
                    ...
                    <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
                      {["Packages", "Services", "Become an Agent"].map(l => (<span key={l} style={{ fontSize: 14, color: "#666", fontWeight: 500 }}>{l}</span>))}
                  ...
                  <div style={{ display: "flex", justifyContent: "center", margin: "0 auto 20px", width: "fit-content" }}>
                    {[{ label: "MTN", bg: flyerColors.mtnColor, txt: "#000" }, { label: "AirtelTigo", bg: flyerColors.airtelColor, txt: "#fff" }, { label: "Telecel", bg: flyerColors.telecelColor, txt: "#fff" }].map((tab, i) => (
              ...
              <p className="text-xs text-muted-foreground text-center">
                Output: {FLYER_W} × {FLYER_H} px. Contact shown: <strong>{supportNum || "— set in Settings"}</strong>
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="withdraw" className="space-y-6 mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="border-yellow-500/30 bg-yellow-500/5"><CardContent className="p-6 text-center space-y-2"><ArrowDownToLine className="h-10 w-10 text-yellow-400 mx-auto" /><p className="text-muted-foreground text-sm">Wallet Balance</p><p className="font-display text-3xl font-bold text-yellow-400">GH₵ {Number(store?.wallet_balance ?? 0).toFixed(2)}</p></CardContent></Card>
            </div>
            <Card className="border-border">
              <CardHeader><CardTitle className="font-display text-lg">Request Withdrawal</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {hasPendingWithdrawal && (<div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3"><p className="text-sm text-yellow-400 font-medium">⚠️ You have a pending withdrawal of GH₵ {pendingWithdrawalAmount.toFixed(2)}. Please wait until it completes before requesting another.</p></div>)}
            ...
            {withdrawals.length > 0 && (
              <Card className="border-border"><CardHeader><CardTitle className="font-display text-lg">Withdrawal History</CardTitle></CardHeader>
                <CardContent><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                  <TableBody>{withdrawals.map(w => (<TableRow key={w.id}><TableCell className="text-sm">{new Date(w.created_at).toLocaleString()}</TableCell><TableCell className="font-bold">GH₵ {Number(w.amount).toFixed(2)}</TableCell><TableCell><Badge className={w.status === "completed" ? "bg-green-600/20 text-green-400 border-green-600/30" : w.status === "pending" ? "bg-yellow-600/20 text-yellow-400 border-yellow-600/30" : "bg-red-600/20 text-red-400 border-red-600/30"}>{w.status}</Badge></TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
            )}
          </TabsContent>
          ...
          <TabsContent value="topup" className="mt-0">
            <Card className="border-border">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-sm"><p className="font-semibold text-yellow-400">⚠️ Important</p><p className="text-muted-foreground">Admin credits your wallet after verifying the transaction ID.</p></div>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="appearance" className="mt-0">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="border-border">
                ...
                <CardContent className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <Textarea value={storeHeadline} onChange={e => setStoreHeadline(e.target.value)} rows={2} placeholder="Get the best data deals from ..." />
                    <Button variant="outline" size="sm" onClick={saveStoreHeadline} disabled={savingHeadline}>{savingHeadline ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Save className="h-4 w-4 mr-1" />}Save Headline</Button>
                  </div>
                    ...
                    {[{ label: "Primary Colour", key: "primary" }, { label: "Text on Primary", key: "primary_foreground" }, { label: "Page Background", key: "background" }, { label: "Card Background", key: "card_background" }].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        ...
                        <div className="flex gap-2 items-center">
                          <Input type="color" value={(themeColors as any)[key]} onChange={e => setThemeColors({ ...themeColors, [key]: e.target.value })} className="w-12 h-9 p-1 cursor-pointer" />
                          <Input type="text" value={(themeColors as any)[key]} onChange={e => setThemeColors({ ...themeColors, [key]: e.target.value })} className="flex-1 font-mono text-sm" />
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="border-t border-border pt-4">
                    <Label className="mb-2 block font-semibold flex items-center gap-2"><LayoutGrid className="h-4 w-4 text-primary" /> Grid Columns</Label>
                    <div className="flex items-center gap-2 max-w-xs">
                      <Button type="button" variant="outline" size="icon" className="h-9 w-9" onClick={() => changeColumns(-1)} disabled={themeColors.gridColumns <= 1}><Minus className="h-4 w-4" /></Button>
                      <Input type="number" min={1} max={6} value={themeColors.gridColumns} onChange={e => { const v = parseInt(e.target.value); if (!isNaN(v) && v >= 1 && v <= 6) setThemeColors({ ...themeColors, gridColumns: v }); }} className="w-20 text-center" />
                      <Button type="button" variant="outline" size="icon" className="h-9 w-9" onClick={() => changeColumns(1)} disabled={themeColors.gridColumns >= 6}><PlusIcon className="h-4 w-4" /></Button>
                      <span className="text-sm text-muted-foreground ml-2">columns per row</span>
                    </div>
                  </div>
                  <div className="flex gap-3 pt-2">
                    <Button variant="hero" onClick={saveThemeColors} disabled={savingTheme} className="flex-1">{savingTheme ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Save className="h-4 w-4 mr-1" />}Save Theme</Button>
                    <Button variant="outline" onClick={resetToDefault} className="flex-1"><RotateCcw className="h-4 w-4 mr-1" />Reset</Button>
                  </div>
                </CardContent>
              </Card>
              ...
              {/* ── Live preview ── */}
              <Card className="border-border">
                <CardHeader><CardTitle className="font-display text-base">Live Preview</CardTitle><p className="text-xs text-muted-foreground">This is exactly how your public store will look.</p></CardHeader>
                <CardContent>
                  <div className="rounded-xl overflow-hidden border border-border" style={{ backgroundColor: themeColors.background, minHeight: 320 }}>
                    {/* Mock store header */}
                    <div className="p-4" style={{ backgroundColor: themeColors.background }}>
                      <div className="text-center mb-3">
                        <p className="font-bold text-sm" style={{ color: themeColors.primary }}>{store?.store_name || "Your Store Name"}</p>
                        <p className="text-xs mt-1" style={{ color: `${themeColors.primary}99` }}>{storeHeadline || "Your store headline"}</p>
                      </div>
                      {/* Mock grid */}
                      <div className="grid gap-2 mt-3" style={{ gridTemplateColumns: `repeat(${Math.min(themeColors.gridColumns, 4)}, minmax(0, 1fr))` }}>
                        {Array.from({ length: Math.min(themeColors.gridColumns * 2, 8) }).map((_, i) => (
                          <div key={i} className="rounded-lg p-2 text-center text-xs" style={{ backgroundColor: themeColors.card_background, border: `1px solid ${themeColors.primary}30` }}>
                            ...
                            <div className="mt-1 rounded text-xs py-0.5 font-bold" style={{ backgroundColor: themeColors.primary, color: themeColors.primary_foreground }}>Buy</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2 text-center">{themeColors.gridColumns} column{themeColors.gridColumns !== 1 ? "s" : ""} per row • Changes apply live after saving</p>
                </CardContent>
              </Card>
            </div>
          </TabsContent>
                ...
                {editingStore ? (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        <Label>Support Number <span className="text-xs text-primary font-normal">(shown on flyer footer)</span></Label>
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <div className="flex items-center justify-between gap-4 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {/* Default ON */}
                            <Switch id="show-group-icon" checked={storeForm.show_whatsapp_group_icon} onCheckedChange={c => setStoreForm({ ...storeForm, show_whatsapp_group_icon: c })} />
                          </div>
                        </div>
                        <Input
                        ...
                        />
                        <p className="text-xs text-muted-foreground">{storeForm.show_whatsapp_group_icon ? "✅ A WhatsApp join icon will appear on your storefront." : "❌ The join icon will be hidden."}</p>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><p className="text-muted-foreground">Show Group Icon</p><p className="font-semibold">{store?.show_whatsapp_group_icon !== false ? "Yes (default)" : "No"}</p></div>
                    ...
                    <div className="col-span-2"><p className="text-muted-foreground">Topup Reference</p><p className="font-display text-xl font-bold text-primary">{store?.topup_reference}</p></div>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <div className="space-y-4 pt-2">
              ...
              {hasPendingWithdrawal && (
                <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-3 text-xs text-orange-400">
                  ⚠️ You have a pending withdrawal of GH₵ {pendingWithdrawalAmount.toFixed(2)}. Wallet balance after buying must not drop below this amount.
                </div>
              )}
