...
const AgentDashboard = () => {
  ...
  const handleBuyConfirm = async () => {
    ...
    if (recentOrders && recentOrders.length > 0) {
      ...
      toast({ title: "Rate limit exceeded", description: `You already purchased data for ${buyPhone.trim()} within the last 45 minutes. Please wait ${45 - elapsed} minute(s).`, variant: "destructive" });
      setBuyLoading(false);
      return;
    }
    ...
    if (buyPaymentMethod === "wallet") {
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke("initialize-payment", {
          body: { email: userEmail, amount: total, phone: buyPhone.trim(), callback_url: callbackUrl, metadata: { package_id: buyPkg.id, network: buyPkg.network, package_name: `${buyPkg.size_gb}GB`, agent_store_id: store.id, payment_method: "paystack", use_agent_price: true } },
        });
  ...
  const handleWithdraw = async () => {
    ...
    if (hasPendingWithdrawal) {
      toast({ title: "Withdrawal already pending", description: "You have a pending withdrawal request. Please wait until it is completed before requesting another.", variant: "destructive" });
      return;
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="overview" className="space-y-6 mt-0">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card className="border-border"><CardContent className="p-6 text-center"><p className="text-muted-foreground text-sm">Revenue (Total sales made)</p><p className="font-display text-2xl font-bold mt-1 text-green-400">GH₵ {profitStats.totalRevenue.toFixed(2)}</p></CardContent></Card>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="border-green-500/30 bg-green-500/5"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Total Profit made from the start of the business</p><p className="font-display text-2xl font-bold text-green-400 mt-1">GH₵ {profitStats.totalProfit.toFixed(2)}</p><p className="text-xs text-muted-foreground mt-1">(Selling Price - Base Price)</p></div><TrendingUp className="h-8 w-8 text-green-400 opacity-50" /></div></CardContent></Card>
          ...
          <TabsContent value="withdraw" className="space-y-6 mt-0">
            ...
            <Card className="border-border">
              ...
              <CardContent className="space-y-4">
                {hasPendingWithdrawal && (<div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-center"><p className="text-sm text-yellow-400 font-medium">⚠️ You have a pending withdrawal request. Please wait until it is completed before requesting another.</p></div>)}
            ...
            {withdrawals.length > 0 && (
              <Card className="border-border">
                ...
                <CardContent>
                  <Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{withdrawals.map((w) => (<TableRow key={w.id}><TableCell className="text-sm">{new Date(w.created_at).toLocaleString()}</TableCell><TableCell className="font-bold">GH₵ {Number(w.amount).toFixed(2)}</TableCell><TableCell><Badge className={w.status === "completed" ? "bg-green-600/20 text-green-400 border-green-600/30" : w.status === "pending" ? "bg-yellow-600/20 text-yellow-400 border-yellow-600/30" : "bg-red-600/20 text-red-400 border-red-600/30"}>{w.status}</Badge></TableCell></TableRow>))}</TableBody></Table>
                </CardContent>
              </Card>
            )}
          </TabsContent>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <ol className="list-decimal list-inside space-y-3 text-sm text-muted-foreground">
                    ...
                    <li>Enter the recipient number: <span className="font-mono font-bold text-foreground">0599449202</span> <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => copyPhoneNumber("0599449202")}><Copy className="h-3 w-3" /></Button></li>
                    <li>Enter the amount you want to add.</li>
                    <li>Use your agent store <strong>top‑up reference</strong> as the transaction reference: <div className="mt-2 p-3 bg-secondary/50 rounded-lg border border-border font-mono font-bold text-center text-primary text-xl">{store?.topup_reference ?? "N/A"} <Button variant="ghost" size="sm" className="ml-2 h-8" onClick={copyRef}><Copy className="h-3 w-3" /> Copy</Button></div></li>
                    <li>Enter your MoMo PIN to confirm the transaction.</li>
                    <li>After successful payment, copy the <strong>transaction ID</strong> from the confirmation message and send it to: <div className="mt-2 flex flex-wrap gap-3"><Button variant="outline" size="sm" asChild><a href="https://wa.me/233200511211" target="_blank" rel="noopener noreferrer">📱 WhatsApp 0200511211</a></Button><Button variant="outline" size="sm" asChild><a href="tel:0599449202">📞 Call 0599449202</a></Button></div><p className="text-xs text-muted-foreground mt-2">If your wallet is not credited after 5 minutes, please call the number above.</p></li>
                  </ol>
                </div>
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-sm"><p className="font-semibold text-yellow-400">⚠️ Important</p><p className="text-muted-foreground">Once you send the transaction ID, the admin will verify and credit your wallet. Your balance will then be available for buying data instantly without any Paystack charges.</p></div>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card className="border-border">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2"><Label>Page Background</Label><div className="flex gap-2 items-center"><Input type="color" value={themeColors.background} onChange={(e) => setThemeColors({ ...themeColors, background: e.target.value })} className="w-16 h-10 p-1" /><Input type="text" value={themeColors.background} onChange={(e) => setThemeColors({ ...themeColors, background: e.target.value })} className="flex-1" placeholder="#0a0a0a" /></div></div>
                  <div className="space-y-2"><Label>Card Background</Label><div className="flex gap-2 items-center"><Input type="color" value={themeColors.card_background} onChange={(e) => setThemeColors({ ...themeColors, card_background: e.target.value })} className="w-16 h-10 p-1" /><Input type="text" value={themeColors.card_background} onChange={(e) => setThemeColors({ ...themeColors, card_background: e.target.value })} className="flex-1" placeholder="#171717" /></div></div>
                </div>
                <div className="border-t border-border pt-4"><h3 className="text-md font-semibold flex items-center gap-2 mb-3"><LayoutGrid className="h-5 w-5 text-primary" /> Product Grid Layout</h3><div className="max-w-xs"><Label className="mb-2 block">Columns per row</Label><div className="flex items-center gap-2"><Button type="button" variant="outline" size="icon" className="h-10 w-10" onClick={() => changeColumns(-1)} disabled={themeColors.gridColumns <= 1}><Minus className="h-4 w-4" /></Button><Input type="number" min={1} max={6} value={themeColors.gridColumns} onChange={(e) => { const val = parseInt(e.target.value); if (!isNaN(val) && val >= 1 && val <= 6) setThemeColors({ ...themeColors, gridColumns: val }); }} className="w-20 text-center" /><Button type="button" variant="outline" size="icon" className="h-10 w-10" onClick={() => changeColumns(1)} disabled={themeColors.gridColumns >= 6}><PlusIcon className="h-4 w-4" /></Button></div><p className="text-xs text-muted-foreground mt-2">How many products appear side by side (1–6).</p></div></div>
                <div className="rounded-lg border border-border p-4 space-y-3 bg-muted/20"><p className="text-sm font-medium">Preview</p><div className="rounded-md p-4" style={{ backgroundColor: themeColors.background }}><div className="rounded-md p-3" style={{ backgroundColor: themeColors.card_background }}><div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${Math.min(themeColors.gridColumns, 4)}, minmax(0, 1fr))` }}>{Array.from({ length: Math.min(themeColors.gridColumns * 2, 8) }).map((_, i) => (<div key={i} className="h-16 rounded-md flex items-center justify-center text-xs" style={{ backgroundColor: themeColors.primary, color: themeColors.primary_foreground }}>Product {i + 1}</div>))}</div><p className="text-xs text-center mt-2 text-muted-foreground">{themeColors.gridColumns} columns</p></div></div></div>
          ...
          <TabsContent value="notifications" className="mt-0 space-y-6">
            <Card className="border-border">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2"><Label>Message</Label><Textarea placeholder="e.g., Special offer: 20% off all data bundles this weekend!" value={newNotificationMsg} onChange={(e) => setNewNotificationMsg(e.target.value)} rows={3} /></div>
                <div className="space-y-2"><Label>Expiry (optional)</Label><Input type="datetime-local" value={newNotificationExpiry} onChange={(e) => setNewNotificationExpiry(e.target.value)} /><p className="text-xs text-muted-foreground">Leave empty for no expiry. After expiry, the notification will no longer be shown.</p></div>
                <Button variant="hero" onClick={createNotification} disabled={sendingNotification}>{sendingNotification ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Plus className="h-4 w-4 mr-1" />} Send Notification</Button>
              </CardContent>
            </Card>
            <Card className="border-border">
              <CardHeader><CardTitle className="font-display">Active & Past Notifications</CardTitle></CardHeader>
              <CardContent>
                {loadingNotifications ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>) : notifications.length === 0 ? (<p className="text-center text-muted-foreground py-8">No notifications yet. Create one above.</p>) : (
                  <div className="space-y-4">{notifications.map((notif) => (<div key={notif.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border border-border rounded-lg bg-card"><div className="flex-1"><p className="text-foreground font-medium">{notif.message}</p><div className="flex flex-wrap gap-3 text-xs text-muted-foreground mt-1"><span>Created: {new Date(notif.created_at).toLocaleString()}</span>{notif.expires_at && (<span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> Expires: {new Date(notif.expires_at).toLocaleString()}</span>)}</div></div><div className="flex gap-2"><Badge variant={notif.is_active ? "default" : "secondary"} className="cursor-pointer hover:opacity-80" onClick={() => toggleNotificationActive(notif.id, notif.is_active)}>{notif.is_active ? "Active" : "Inactive"}</Badge><Button variant="ghost" size="icon" onClick={() => deleteNotification(notif.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></div>))}</div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={buyPaymentMethod} onValueChange={(v) => setBuyPaymentMethod(v as "paystack" | "wallet")}>
                  ...
                  <SelectContent>
                    <SelectItem value="wallet"><span className="flex items-center gap-2"><Wallet className="h-4 w-4" /> Wallet (Balance: GH₵ {store?.wallet_balance?.toFixed(2) ?? '0.00'})</span></SelectItem>
                    <SelectItem value="paystack"><span className="flex items-center gap-2"><CreditCard className="h-4 w-4" /> Paystack (+ charges)</span></SelectItem>
                  </SelectContent>
                </Select>
              </div>
