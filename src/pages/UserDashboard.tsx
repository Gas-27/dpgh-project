...
// Check for rate limit (4 minute window)
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
          {miniTabs.map((tab) => (
            <button
              ...
              className={`rounded-full border px-3 py-2 text-sm font-semibold transition hover:brightness-95 ${tab.className}`}
            >
              {tab.label}
            </button>
          ))}
  ...
  const renderOverview = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2"><SpinToWinCard target="packages" /></div>
