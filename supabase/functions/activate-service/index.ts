...
Deno.serve(async (req) => {
  ...
  try {
    const { phone, pin, service_id, free_service } = await req.json();
    ...
    if (!order && free_service) {
      const { data: slot } = await db.from("digital_service_credentials").select("id").eq("service_id", service_id).eq("active", true).is("assigned_order_id", null).order("slot_number").limit(1).maybeSingle();
      if (slot) {
