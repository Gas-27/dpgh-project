...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const results = await Promise.all(recipients.map(async (to: string) => {
      try {
        const response = await fetch("https://api.txtconnect.net/dev/api/sms/send", { method: "POST", headers: providerHeaders, body: JSON.stringify({ to: toTxtConnectNumber(to) || to, from: senderId, unicode: unicode ? "1" : "0", sms: message }) });
        const raw = await response.text();
