...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ============================================
    const rawNetwork = network.toLowerCase().trim().replace(/[\s-]+/g, "_");
    ...
    const routeSizeGb = Number(size_gb);
    const fallbackProvider = NETWORK_TO_PROVIDER[normalizedNetwork];
    const { data: mappedProvider, error: routeError } = await supabase.rpc("get_network_provider_route_for_size", { p_network_key: normalizedNetwork, p_flow: "purchase", p_size_gb: routeSizeGb });
